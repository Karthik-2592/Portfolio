...
export function NavigationBar() {
  ...
  return (
    <nav
      ref={navRef}
      className={`fixed top-0 left-0 right-0 h-16 z-50 glass shadow-lg shadow-black/20`}
    >
      <div className="max-w-7xl mx-auto px-6 h-full">
        <div className="flex items-center justify-between h-full">
          <a href="#" className="nav-logo text-2xl text-white hover:text-primary transition-colors duration-300 hover:scale-110 active:scale-100 transform">
            <svg
              width="48" height="48" version="1.1" viewBox="0 0 156.8 156.8"
          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="nav-link text-sm font-medium text-muted-foreground hover:text-primary transition-colors duration-300 relative group"
              >
                {link.label}
